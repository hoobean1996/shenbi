...
export function useLiveSession({
  ...
  const studentResponseToState = useCallback((response: LiveSessionStudentResponse): LiveSessionState => {
  ...
  const apiResponseToState = useCallback((response: LiveSessionResponse, currentRole: LiveSessionRole): LiveSessionState => {
    // Map API status to our status
    let status: LiveSessionStatus;
    switch (response.status) {
      case 'waiting':
  ...
  // Poll session state
  const pollSessionState = useCallback(async (classroomId: number, currentRole: LiveSessionRole) => {
    try {
  ...
  // Start polling
  const startPolling = useCallback((classroomId: number, currentRole: LiveSessionRole, interval: number) => {
  ...
  // Start a new session
  const startSession = useCallback(async (classroomId: number) => {
    if (role !== 'teacher') {
  ...
  // Set the level
  const setLevel = useCallback(async (level: LevelDefinition) => {
    ...
    try {
      await liveSessionApi.setLevel(state.classroomId, { level: level as unknown as Record<string, unknown> });
      // Refresh state
  ...
  const joinSession = useCallback(async (classroomId: number) => {
    if (role !== 'student') {
    ...
    try {
      const response = await liveSessionApi.join(classroomId, { student_name: userName }) as LiveSessionStudentResponse;
  ...
  const joinByCode = useCallback(async (roomCode: string) => {
  ...
  // Update progress
  const updateProgress = useCallback(async (starsCollected: number, completed: boolean, code?: string) => {
    if (!state.classroomId) {
      return;
    }
